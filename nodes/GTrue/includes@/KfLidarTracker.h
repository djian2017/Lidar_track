...
public:
	...
	CTrack(Cluster& in_cluster, float in_time_delta, float in_acceleration_noise_magnitude, size_t in_track_id)
		: kf_(cv::Point2f((in_cluster.GetCentroid()).x, (in_cluster.GetCentroid()).y), in_time_delta, in_acceleration_noise_magnitude)
	{
		track_id = in_track_id;
		skipped_frames = 0;
		prediction_point_ = cv::Point2f((in_cluster.GetCentroid()).x, (in_cluster.GetCentroid()).y);
		cluster = in_cluster;
		life_span = 0;
		area = 0;
	}
	...
	void Update(Cluster& in_cluster, bool in_data_correct, size_t in_max_trace_length)
	{
		kf_.GetPrediction();
		prediction_point_ = kf_.Update(cv::Point2f((in_cluster.GetCentroid()).x, (in_cluster.GetCentroid()).y), in_data_correct);
